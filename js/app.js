...
*/
    ...
    const words = gsapWithCSS.utils.toArray(".main__line");
    ...
    splits.forEach(((split, i) => {
        if (i) gsapWithCSS.set(split.chars, {
            yPercent: 100
        });
        ...
        if (next) tl.to(next.chars, {
            duration,
            yPercent: 0,
            stagger: staggerTime
        }, "<");
    }));
    tl.fromTo(firstSplit.chars, {
        yPercent: 100
    }, {
        duration,
        yPercent: 0,
    ...
    document.addEventListener("click", (e => {
        const contactButton = e.target.closest(".main__button");
        const closeButton = e.target.closest(".contacts__button");
        if (contactButton) toggleScreenState(true);
        if (closeButton) toggleScreenState(false);
    }));
